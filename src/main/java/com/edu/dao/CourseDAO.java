...
import com.edu.utils.model.User;
...
public class CourseDAO {
    ...
    public List getCourseByPage(Page page) {
        return getSession().createQuery("select c.courseId,c.courseName,c.courseDescription,c.coursePic from Course c order by startTime desc")
    ...
    public List getSharpCourseByTeacherId(String teacherId) {
        return getSession()
                .createQuery("select c.id,c.courseName,c.courseDescription from Course c" +
                        " where c.teacher.userId=?")
                .setString(0, teacherId)
                .list();
    }
